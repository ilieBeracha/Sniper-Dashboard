...
export class TrainingPDFExporter {
  ...
  private generateTitlePage(training: TrainingSession): void {
    ...
    // this.doc.text(`Conducted by: ${training.creator_id.first_name} ${training.creator_id.last_name}`, centerX, this.currentY, { align: "center" });
    ...
    this.doc.setTextColor(this.styles.colors.secondary);
    this.doc.text(
      `Report generated on ${new Date().toLocaleDateString("en-US")} at ${new Date().toLocaleTimeString("en-US")}`,
      centerX,
      this.currentY,
      { align: "center" },
    );
  ...
  private generateExecutiveSummary(data: TrainingReportData): void {
    ...
    const kpiData = [
      ...
      [
        ...
        `${data.analytics.overall_hit_percentage?.toFixed(1)}%`,
        this.getPerformanceAssessment(data.analytics.overall_hit_percentage),
      ],
      ...
      [
        ...
        `${data.analytics.avg_time_to_first_shot?.toFixed(2)}s`,
        this.getTimeAssessment(data.analytics.avg_time_to_first_shot),
      ],
  ...
  private generateTrainingDetails(training: TrainingSession, participants: TrainingParticipant[]): void {
    ...
    if (training.assignments && training.assignments.length > 0) {
      ...
      const assignmentData = [["Assignment Name", "Created Date"]];

      training.assignments.forEach((assignment) => {
        assignmentData.push([assignment.assignment_name, new Date(assignment.created_at).toLocaleDateString("en-US")]);
      });
  ...
  private generateParticipantsOverview(participants: TrainingParticipant[]): void {
    ...
    const participantData = [["Name", "Role", "Email", "Participation Date"]];
    ...
    Object.entries(roleCount).forEach(([role, count]) => {
      ...
      roleData.push([role.charAt(0).toUpperCase() + role.slice(1), count.toString(), `${percentage}%`]);
    });
  ...
  private generatePerformanceAnalytics(analytics: TrainingTeamAnalytics): void {
    ...
    const overallData = [["Metric", "Value", "Details"]];
    ...
    const performanceMetrics = [
      ...
      [
        "Average First Shot Time",
        `${analytics.avg_time_to_first_shot?.toFixed(2)} seconds`,
        this.getTimeAssessment(analytics.avg_time_to_first_shot),
      ],
    ...
    const distanceData = [["Distance Category", "Shots Fired", "Hit Percentage", "Assessment"]];
  ...
  private generateSquadPerformance(squadStats: SquadStats[], squadPerformance: SquadPerformance[]): void {
    ...
    if (squadStats.length > 0) {
      const squadData = [["Name", "Squad", "Role/Weapon", "Hit %", "Sessions"]];
    ...
    if (squadPerformance.length > 0) {
      ...
      const squadSummaryData = [["Squad", "Missions", "Avg Accuracy", "Avg Reaction Time", "First Shot Success", "Elimination Rate", "Best Sniper"]];
  ...
  private generateWeaponEquipmentAnalysis(weaponStats: WeaponUsageStats[], weapons: Weapon[], equipment: Equipment[]): void {
    ...
    if (weaponStats.length > 0) {
      ...
      const weaponData = [["Weapon ID", "Shots Fired", "Hits", "Hit Percentage", "Avg Dispersion", "Best Dispersion"]];
    ...
    if (weapons.length > 0) {
      ...
      const weaponInventoryData = [["Weapon Type", "Serial Number", "Muzzle Velocity"]];

      weapons.forEach((weapon) => {
        weaponInventoryData.push([weapon.weapon_type, weapon.serial_number, weapon.mv || "N/A"]);
      });
    ...
    if (equipment.length > 0) {
      ...
      const equipmentData = [["Equipment Type", "Serial Number", "Day/Night Configuration"]];

      equipment.forEach((equip) => {
        equipmentData.push([equip.equipment_type, equip.serial_number, equip.day_night.charAt(0).toUpperCase() + equip.day_night.slice(1)]);
      });
  ...
  private generateDayNightAnalysis(dayNightPerformance: DayNightPerformance[]): void {
    ...
    if (dayNightPerformance.length > 0) {
      const dayNightData = [["Condition", "Total Missions", "Accuracy", "First Shot Success", "Avg Reaction Time", "Elimination Rate"]];
      ...
      dayNightPerformance.forEach((performance) => {
        dayNightData.push([
          ...
          performance.accuracy?.toString(),
          performance.first_shot_success_rate?.toString(),
          performance.avg_reaction_time?.toString(),
          performance.elimination_rate?.toString(),
        ]);
      });
    ...
    if (effectiveness.length > 0) {
      ...
      effectiveness.forEach((eff) => {
        effectivenessData.push([
          ...
          eff.avg_accuracy_improvement ? `${eff.avg_accuracy_improvement?.toFixed(1)}%` : "N/A",
          eff.avg_reaction_time_improvement ? `${eff.avg_reaction_time_improvement?.toFixed(2)}s` : "N/A",
  ...
  private generateRecommendations(data: TrainingReportData): void {
    ...
    const worstPerformingSquad = data.squadStats.reduce((worst, current) => (current.hit_percentage < worst.hit_percentage ? current : worst));
