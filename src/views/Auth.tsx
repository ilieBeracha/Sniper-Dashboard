...
export default function Auth() {
  ...
  return (
    <div className={`flex flex-col md:flex-row h-[100dvh] overflow-hidden transition-colors duration-200 ${theme === "dark" ? "bg-[#121212]" : "bg-gray-100"}`}>
      <div
      ...
      <div
        className={`flex-1 md:flex-none w-full md:w-3/5 flex items-center justify-center p-4 sm:p-6 md:p-8 relative z-10 transition-all duration-200 ${
          theme === "dark" ? "md:shadow-black md:shadow-2xl" : "md:shadow-gray-300 md:shadow-lg"
        }`}
      >
        <div className="w-full max-w-sm">
          <div className="mb-6">
            <h2 className={`text-base sm:text-lg font-semibold mb-1 transition-colors duration-200 ${theme === "dark" ? "text-white" : "text-gray-900"}`}>
              {getAuthTitle()}
            </h2>
            <p className={`text-xs sm:text-sm transition-colors duration-200 ${theme === "dark" ? "text-gray-500" : "text-gray-600"}`}>{getAuthDescription()}</p>
          </div>

          {/* Form Card - Compact */}
          <div className="relative">
            <div
              className={`relative py-3 px-3 sm:py-4 sm:px-4 rounded-xl border transition-colors duration-200 ${
                theme === "dark" ? "border-[#2A2A2A] bg-black/20" : "border-gray-300 bg-white/80 backdrop-blur-sm"
              }`}
            >
              {isLoading && (
                <div
                  className={`absolute inset-0 flex items-center justify-center z-10 rounded-xl backdrop-blur-sm transition-colors duration-200 ${
                ...
                >
                  <div className="flex flex-col items-center">
                    <div className="relative">
                      <div
                        className={`w-10 h-10 border-2 rounded-full transition-colors duration-200 ${
                          theme === "dark" ? "border-gray-700" : "border-gray-300"
                        }`}
                      />
                      <div
                        className={`absolute inset-0 w-10 h-10 border-t-2 rounded-full animate-spin transition-colors duration-200 ${
                          theme === "dark" ? "border-white" : "border-gray-700"
                        }`}
                      />
                    </div>
                    <p className={`mt-3 text-xs transition-colors duration-200 ${theme === "dark" ? "text-white" : "text-gray-700"}`}>
                      Authenticating...
                    </p>
                  </div>
                </div>
                ...
                {authType !== "login" && (
                  <div>
                    <p
                      ...
                      className={`mb-3 text-xs cursor-pointer transition-colors flex items-center ${
              ...
              {error && authType !== "login" && (
                <div className="mt-4 p-3 bg-red-900/10 border border-red-900/20 rounded-lg flex items-center">
                  <div className="w-6 h-6 bg-red-900/20 rounded-full flex items-center justify-center mr-2 flex-shrink-0">
