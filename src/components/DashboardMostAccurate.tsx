            ...
            <BaseDashboardCard title="Precision Accuracy">
                <ol className="space-y-3 mt-4 h-full overflow-y-auto">
                    {(topAccurateSnipers || []).slice(0, 5).map((sniper: any, idx: number) => {
                        ...
                        return (
                            <li
                                key={sniper.user_shooter_name + idx}
                                className="space-y-1 px-2 rounded transition"
                            >
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center justify-start">
                                        ...
                                        <span className={`text-zinc-100 font-medium text-lg flex flex-col`}>
                                            ...
                                            {
                                                idx === 0 ? (
                                                    <p className="text-zinc-400 text-xs ">Most Accurate</p>
                                                ) : null
                                            }
                                        </span>
                                    </div>
                                ...
                                <div className="w-full h-1.5 bg-zinc-800 rounded">
                                    <div
                                        className="h-full bg-zinc-400 rounded transition-all mt-4"
                    ...
                    {(!topAccurateSnipers || topAccurateSnipers.length === 0) && (
                        <li className="text-zinc-500 text-xs italic px-2 py-2">No data available.</li>
                    )}
                </ol>
            </BaseDashboardCard>
