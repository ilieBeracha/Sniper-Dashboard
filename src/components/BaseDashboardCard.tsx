...
export default function BaseDashboardCard({
  ...
  const cardClassName = `relative h-full flex flex-col transition-all text-sm duration-300 rounded-lg border ${
    theme === 'dark' 
      ? 'bg-[#1A1A1A] border-white/10 shadow-lg shadow-black/20' 
      : 'bg-white border-gray-200 shadow-sm shadow-gray-200/50'
  } ${isMobile ? "" : "h-full"}`;
  ...
  return (
    <div className={cardClassName}>
      <div className={`${padding} border-b mb-2 transition-colors duration-200 ${
        ...
        <div className="flex justify-between relative h-full items-center">
          {tooltipContent && (
            <BiInfoCircle
              className={`absolute top-0 -right-1 cursor-help transition-colors duration-200 ${
                theme === 'dark' 
                  ? 'text-indigo-400/80 hover:text-indigo-400' 
                  : 'text-indigo-600/80 hover:text-indigo-600'
              }`}
              size={16}
          ...
          <h2 className={`font-semibold flex items-center gap-2 text-sm relative transition-colors duration-200 ${
          ...
          {withBtn ?? (
            <button className={`p-1.5 rounded-lg transition-colors duration-200 ${
              ...
              <BiAddToQueue className={`transition-colors duration-200 ${
