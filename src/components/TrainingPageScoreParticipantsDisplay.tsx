    ...
    <div className="border border-zinc-700 rounded-lg overflow-hidden">
      ...
      <div className="divide-y divide-zinc-700/50">
        {participants.map((participant: ScoreParticipant, index: number) => (
          <div key={`${participant.user_id}-${index}`} className="p-3 sm:p-4 bg-zinc-800/50">
            <div className="grid grid-cols-1 gap-3 sm:gap-4 md:grid-cols-2">
              {/* User Info Card */}
              <div className="bg-zinc-800 rounded-lg p-3 sm:p-4 border border-zinc-700/50">
                <div className="flex items-center mb-2 sm:mb-3">
                  <Users className="h-4 w-4 sm:h-5 sm:w-5 text-zinc-400 mr-2" />
              ...
              <div className="bg-zinc-800 rounded-lg p-3 sm:p-4 border border-zinc-700/50">
                <div className="space-y-2 sm:space-y-3">
                  ...
                  <div className="text-white text-sm sm:text-base font-medium ml-6 break-words">{getWeaponInfo(participant.weapon_id)}</div>
                  ...
                  <div className="text-white text-sm sm:text-base font-medium ml-6 break-words">{getEquipmentInfo(participant.equipment_id)}</div>
                </div>
              </div>
            </div>
          </div>
