...
export const TargetsSection = ({ section, targets, addTarget, updateTarget, removeTarget }: TargetsSectionProps) => {
  ...
  return (
    <div id="targets" className="snap-start scroll-mt-4 h-[calc(100vh-5rem)] flex flex-col space-y-6 py-8">
      ...
      <div
        className={`border ${theme === "dark" ? "border-white/10 bg-zinc-900/30" : "border-gray-200 bg-gray-50/50"} rounded-lg flex-1 overflow-auto p-4`}
      >
        <div className="space-y-8 lg:space-y-10">
            ...
            <div key={target.id} className="p-6 lg:p-10">
              <div className="flex sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div className="flex items-center space-x-3">
                  <div
                    className={`w-10 h-10 bg-gradient-to-br ${theme === "dark" ? "from-zinc-700 to-zinc-800" : "from-gray-200 to-gray-300"} rounded-lg flex items-center justify-center text-base font-medium ${theme === "dark" ? "text-gray-300" : "text-gray-700"} flex-shrink-0 shadow-sm`}
                  >
                    {index + 1}
                  </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-2 gap-6 lg:gap-8">
                  <div className="space-y-3">
                    <Label className={`text-sm sm:text-xs lg:text-base font-medium ${theme === "dark" ? "text-gray-300" : "text-gray-700"}`}>
                      Wind Speed (m/s)
                    </Label>
                    <Input
                      type="number"
                ...
                <div className="space-y-2 lg:col-span-2">
                  <Label className={`text-base sm:text-xs lg:text-base font-medium ${theme === "dark" ? "text-gray-300" : "text-gray-700"}`}>
                    Mistake Code
                  </Label>
                  <Input
                    placeholder="Optional"
                    ...
                    className={`w-full h-10 border-gray-300 ${theme === "dark" ? "border-white/10" : "border-gray-300"} focus:border-indigo-500 dark:focus:border-indigo-400 focus:ring-1 focus:ring-indigo-500 dark:focus:ring-indigo-400 dark:bg-zinc-800 dark:text-white transition-all`}
                  />
                </div>
              </div>
            </div>
          ...
          {targets.length === 0 && (
            <div
              className={`text-center py-12 border ${theme === "dark" ? "border-white/10 bg-zinc-900/30" : "border-gray-200 bg-gray-50/50"} rounded-lg p-4`}
            >
