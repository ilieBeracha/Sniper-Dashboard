...
const UserGroupingSummary = () => {
  ...
  const trend = lastGroups.length >= 2 ? (lastGroups[0].cm_dispersion < lastGroups[1].cm_dispersion ? "improving" : "declining") : "stable";
    ...
    <div className="flex flex-col h-full w-full p-4">
        ...
        <div className="relative w-48 h-48">
          <ResponsiveContainer width="100%" height="100%">
            <RadialBarChart cx="50%" cy="50%" innerRadius="60%" outerRadius="90%" data={radialData}>
              ...
              <RadialBar dataKey="value" cornerRadius={10} fill={radialData[0].fill} animationDuration={1000} />
            </RadialBarChart>
          </ResponsiveContainer>

          {/* Center content */}
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <div className={`text-3xl font-bold ${theme === "dark" ? "text-white" : "text-gray-900"}`}>{performanceScore.toFixed(0)}%</div>
            ...
            <div className="flex items-center mt-1">
              ...
              <span className={`text-xs ${trend === "improving" ? "text-green-500" : trend === "declining" ? "text-red-500" : "text-gray-500"}`}>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className={`p-3 rounded-lg ${theme === "dark" ? "bg-gray-800/50" : "bg-gray-50"}`}>
          ...
          <div className={`text-lg font-semibold mt-1 text-green-500`}>{groupingSummary.best_dispersion ?? "-"} cm</div>
        </div>
        ...
        <div className={`p-3 rounded-lg ${theme === "dark" ? "bg-gray-800/50" : "bg-gray-50"}`}>
          ...
          <div className={`text-lg font-semibold mt-1 text-amber-500`}>{groupingSummary.avg_time_to_group ?? "-"}s</div>
        </div>
        ...
        <div className={`p-3 rounded-lg ${theme === "dark" ? "bg-gray-800/50" : "bg-gray-50"}`}>
          ...
          <div className={`text-lg font-semibold mt-1 text-blue-500`}>{groupingSummary.total_groupings ?? 0}</div>
        </div>
      </div>
    </div>
  );
