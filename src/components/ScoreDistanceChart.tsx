...
export default function ScoreDistanceChart({ rows }: { rows: ScoreTarget[] | undefined }) {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div
          className={`rounded-md border p-3 shadow-lg backdrop-blur-sm transition-colors duration-200 ${theme === "dark" ? "border-white/10 bg-black/80" : "border-gray-200 bg-white/90"}`}
        >
          <p
        ...
        {data.length > 0 ? (
          <ResponsiveContainer minHeight={300} maxHeight={400} width="100%" height="100%">
            <BarChart data={data} margin={{ top: 30, right: 30, left: 0, bottom: 5 }} barGap={2}>
              <CartesianGrid strokeDasharray="3 3" stroke={theme === "dark" ? "#353535" : "#e5e7eb"} vertical={false} />
              <XAxis
                dataKey="distance"
              ...
              <Legend
                ...
                formatter={(value) => (
                  <span className={`transition-colors duration-200 ${theme === "dark" ? "text-gray-300" : "text-gray-600"}`}>{value}</span>
                )}
              />
          ...
          <div className="flex h-full w-full flex-col items-center justify-center">
            <div className={`mb-2 rounded-full p-3 transition-colors duration-200 ${theme === "dark" ? "bg-gray-800" : "bg-gray-200"}`}>
              <svg
                className={`h-6 w-6 transition-colors duration-200 ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
