  ...
  const SidebarContent = () => (
    ...
    >
      ...
      <div className={`flex items-center justify-between h-16 px-4 border-b transition-colors duration-200 ${
        theme === 'dark' 
          ? 'border-[#1D1D1F] bg-[#121212]' 
          : 'border-gray-200 bg-white'
      }`}>
        {!collapsed && <span className={`text-lg font-bold transition-colors duration-200 ${
          theme === 'dark' ? 'text-white' : 'text-gray-900'
        }`}>ScopeStats</span>}
        <button 
          onClick={() => (isMobile ? toggleDrawer() : setCollapsed(!collapsed))} 
          className={`text-xl hover:opacity-80 transition-colors duration-200 ${
      ...
      {!collapsed && (
        <div className="px-4 pt-4">
          <div className={`p-3 rounded-lg flex items-center space-x-3 transition-colors duration-200 ${
      ...
      <div className="flex-1 px-2 py-4 overflow-y-auto transition-all">
        {navSections.map(({ title, items }) => (
          <div key={title} className="mb-4">
            {!collapsed && <h4 className={`text-xs px-4 mb-1 tracking-wide transition-colors duration-200 ${
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center px-4 py-2 text-sm rounded-lg transition-colors duration-150 ${
                    isActive 
                      ? (theme === 'dark' ? "bg-[#1E1E20] text-white" : "bg-gray-100 text-gray-900")
                      : (theme === 'dark' 
      ...
      <>
        <Dialog open={isDrawerOpen} onClose={() => toggleDrawer()} className="relative z-50">
          <div className={`fixed inset-0 transition-opacity duration-300 ${
            theme === 'dark' ? 'bg-black/30' : 'bg-gray-900/30'
          }`} aria-hidden="true" />
          <div className={`fixed inset-y-0 left-0 max-w-xs shadow-xl transform transition-transform duration-300 ease-in-out ${
