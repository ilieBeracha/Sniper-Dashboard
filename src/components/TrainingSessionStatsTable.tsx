...
import { getSessionStatsCountByTrainingId } from "@/services/sessionService";
...
export default function TrainingSessionStatsTable({ onScoreClick, onEditClick, newlyAddedScoreId }: TrainingScoresTableProps) {
  ...
  const loadScores = async (page: number = 0, reset: boolean = false) => {
    ...
    try {
      ...
      const result = await getSessionStatsByTrainingId(id, offset, SCORES_LIMIT);
  ...
  useEffect(() => {
    if (sessionStats?.length > 0 && currentPage === 0) {
      ...
      setPaginatedScores(sessionStats);
    }
  }, [sessionStats?.length]);
  ...
  //   const positions = sessionStats.map((score) => score.position).filter(Boolean);
  //   return [...new Set(positions)];
  // }, [sessionStats]);

  // const uniqueDayNight = useMemo(() => {
  //   const dayNight = sessionStats.map((score) => score.day_night).filter(Boolean);
