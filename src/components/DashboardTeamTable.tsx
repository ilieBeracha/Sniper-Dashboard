...
export default function DashboardTeamTable() {
  ...
  const RoleBadge = ({ role }: { role: string }) => {
    ...
    return (
      <div className={`px-3 py-1 rounded-md text-xs sm:text-sx inline-block border transition-colors duration-200 ${getBadgeColor()}`}>{role}</div>
    );
  };
    ...
    <>
      ...
      >
            ...
            <thead className={`transition-colors duration-200 ${theme === "dark" ? "bg-dashboard-card" : "bg-gray-50"}`}>
              <tr>
                {["ID", "Operative", "Squad", "Role", "Actions"].map((header, i) => (
                  <th
                    key={i}
                    className={`px-6 py-3 text-left text-xs sm:text-sx font-semibold uppercase tracking-wider transition-colors duration-200 ${theme === "dark" ? "text-white/70" : "text-gray-700"}`}
                  >
                    {header === "ID" ? (
            ...
            <tbody className={`divide-y transition-colors duration-200 ${theme === "dark" ? "divide-white/10" : "divide-gray-200"}`}>
              {members?.map((member: User) => {
                ...
                return (
                  <tr
                    key={rowId}
                    className={`transition-colors text-sx ${selectedRows.includes(rowId) ? (theme === "dark" ? "bg-white/10" : "bg-blue-50") : ""} ${
                      hoveredRow === rowId
                        ? theme === "dark"
                          ? "bg-white/10"
                          : "bg-gray-50"
                  ...
                  >
                    <td
                      className={`px-6 py-4 whitespace-nowrap text-sx transition-colors duration-200 ${theme === "dark" ? "text-dashboard-text" : "text-gray-700"}`}
                    >
                      <div className="flex items-center gap-3">
                    ...
                    <td
                      className={`px-6 py-2 whitespace-nowrap text-sx transition-colors duration-200 ${theme === "dark" ? "text-white" : "text-gray-900"}`}
                    >
                      {isCommander(user?.user_role as UserRole) ? (
                        <div className="flex items-center space-x-2">
                          <BaseButton
                            onClick={() => {}}
                            className={`p-1.5 rounded-md transition-colors duration-200 ${theme === "dark" ? "hover:bg-white/10" : "hover:bg-gray-100"}`}
                          >
                            <BiPencil
                              className={`size-4 hover:text-blue-400 transition-colors duration-200 ${theme === "dark" ? "text-dashboard-text-muted" : "text-gray-600"}`}
                            />
                          </BaseButton>
                          <BaseButton
                            onClick={() => {}}
                          ...
                          >
                            <BiTrash
                              className={`size-5 hover:text-[#F25F4C] transition-colors duration-200 ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}
                            />
                          </BaseButton>
                          <BaseButton
                            onClick={() => {}}
                          ...
                          >
                            <BiDotsHorizontalRounded
                              className={`size-4 transition-colors duration-200 ${theme === "dark" ? "text-dashboard-text-muted hover:text-white" : "text-gray-600 hover:text-gray-900"}`}
                            />
                          </BaseButton>
                        </div>
                      ) : (
                        <span className={`text-sx italic transition-colors duration-200 ${theme === "dark" ? "text-gray-500" : "text-gray-400"}`}>
                          —
                        </span>
                      )}
                    </td>
        ...
        {(!members || members.length === 0) && (
          <div
            className={`flex flex-col items-center justify-center py-16 text-center transition-colors duration-200 ${theme === "dark" ? "text-dashboard-text-muted" : "text-gray-600"}`}
          >
            ...
            <p className={`text-xs sm:text-sx transition-colors duration-200 ${theme === "dark" ? "text-dashboard-text-soft" : "text-gray-500"}`}>
