...
export default function WeaponUsageModal({ isOpen, onClose, weapon }: WeaponUsageModalProps) {
  ...
  const cardBg = theme === "dark" 
    ? "bg-gradient-to-br from-zinc-900/95 to-zinc-800/95 border-zinc-700/50" 
          ...
          {isLoading ? (
            ...
            <div className="space-y-6">
              ...
              <div className={`p-4 rounded-lg ${
                theme === "dark" ? "bg-zinc-800/40" : "bg-gray-50"
              }`}>
                ...
                <p className={`text-sm ${
                  ...
                  <span className="font-medium">{weaponUsageStats.total_shots_fired || 0}</span> shots fired,
                  achieving a <span className="font-medium">{weaponUsageStats.hit_percentage ? weaponUsageStats.hit_percentage.toFixed(1) : 0}%</span> hit rate.
                  {weaponUsageStats.avg_cm_dispersion && (
                    <> Average grouping dispersion is{" "}
                    <span className="font-medium">{weaponUsageStats.avg_cm_dispersion.toFixed(1)}cm</span>.</>
                  )}
                </p>
              </div>
            </div>
...
function StatCard({ icon, label, value, theme, colorScheme, highlight = false, fullWidth = false }: StatCardProps) {
  const getColorClasses = () => {
    const colors = {
      blue: {
        bg: theme === "dark" 
          ? highlight ? "bg-blue-500/10 border border-blue-500/20" : "bg-blue-500/5"
          : highlight ? "bg-blue-50 border border-blue-100" : "bg-blue-25",
      ...
      green: {
        bg: theme === "dark" 
          ? highlight ? "bg-green-500/10 border border-green-500/20" : "bg-green-500/5"
          : highlight ? "bg-green-50 border border-green-100" : "bg-green-25",
      ...
      purple: {
        bg: theme === "dark" 
          ? highlight ? "bg-purple-500/10 border border-purple-500/20" : "bg-purple-500/5"
          : highlight ? "bg-purple-50 border border-purple-100" : "bg-purple-25",
      ...
      gold: {
        bg: theme === "dark" 
          ? highlight ? "bg-amber-500/10 border border-amber-500/20" : "bg-amber-500/5"
          : highlight ? "bg-amber-50 border border-amber-100" : "bg-amber-25",
